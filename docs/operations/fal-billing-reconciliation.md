...
```bash
install -m 600 /dev/null /etc/pias/fal-inference.key
install -m 600 /dev/null /etc/pias/fal-admin.key
```
...
- 后台 Worker 按 `PIAS_FAL_BILLING_RETRY_MS` 冷却周期重试 `pending` 和 `unavailable` 记录，配置修复后无需手工改数据。
- 只有全部上游 request ID 均获得 Billing Event 时才标记 `confirmed`；部分返回不得冒充完成。
- 客户额度与供应商成本允许口径不同：PIAS V1 失败或取消默认释放客户额度，但供应商已产生成本仍会记入内部对账。
