...
npm run migrate:project-storage -- \
  --asset-source /var/lib/pias/assets \
  --asset-target-root /var/lib/pias/asset-scopes \
  --fal-source /var/lib/pias/fal-queue-state.json \
  --fal-target-root /var/lib/pias/fal-queue-scopes \
  --tenant tenant-a \
  --project project-main
```
...
2. 部署迁移前构建，移除 `PIAS_AUTH_CONFIG_FILE`，继续使用未被迁移命令修改的 `PIAS_STUDIO_STATE_FILE`。
3. 若分区环境已经产生新写入，不得直接覆盖旧文件；先导出并人工合并差异，再决定恢复点。
4. 回滚验证完成前禁止同时启动旧单文件实例和新分区实例写入同一业务项目。

该回滚不删除任何状态文件。回滚旧构建时同时恢复旧素材目录与 `PIAS_FAL_JOB_STATE_FILE`；已分区目录保留用于核对新旧差异。
