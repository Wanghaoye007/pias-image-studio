# Content Studio SQLite 数据库运维手册

## 适用范围

启用企业身份配置后，StudioState、Fal 作业、Worker 租约和两阶段恢复载荷默认使用 SQLite。本文覆盖首次启用、旧范围快照迁移、在线备份、停机恢复和回滚。素材原文件仍位于 `CONTENT_STUDIO_ASSET_DIR`，必须与数据库使用同一备份批次。
...
- 同一主机可运行多个 Content Studio 进程；SQLite 使用 WAL、`synchronous=FULL`、5 秒 busy timeout 和 `BEGIN IMMEDIATE` 保证 revision 更新串行化。
...
CONTENT_STUDIO_DATABASE_FILE=/var/lib/content-studio/content-studio.sqlite
...
CONTENT_STUDIO_FAL_WORKER_INTERVAL_MS=2500
CONTENT_STUDIO_FAL_LEASE_TTL_MS=15000
```

## 旧数据迁移

1. 停止 Content Studio 写入，先按 `tenant-state-migration.md` 将旧单文件拆分到范围目录。
2. 执行 dry-run；该步骤不创建数据库：

```bash
npm run migrate:sqlite -- \
  --studio-root /var/lib/content-studio/studio-state-scopes \
  --fal-root /var/lib/content-studio/fal-queue-scopes \
  --database /var/lib/content-studio/content-studio.sqlite
```
...
迁移回滚：停止服务，设置 `CONTENT_STUDIO_PERSISTENCE_BACKEND=file`，继续使用原 `CONTENT_STUDIO_STATE_DIR`。禁止 SQLite 与旧文件后端同时写入同一项目。
...
npm run database -- backup \
  --database /var/lib/content-studio/content-studio.sqlite \
  --output /var/backups/content-studio/content-studio-$(date +%Y%m%d-%H%M%S).sqlite
```

备份成功输出 `integrity: ok`、`sha256` 和 `.manifest.json` 路径。备份任务失败时不得删除上一次成功副本。建议每日全量备份、保留 30 天，并将同批次数据库、素材目录和身份配置的加密副本写入独立介质。

发布前将本次备份路径写入 `CONTENT_STUDIO_RELEASE_BACKUP_FILE`。统一预检会校验备份与清单均为 `0600`、SHA-256 和完整性有效，并确认清单 `source` 对应当前 `CONTENT_STUDIO_DATABASE_FILE`，防止拿无关健康数据库冒充回滚点：

```bash
export CONTENT_STUDIO_RELEASE_BACKUP_FILE=/var/backups/content-studio/content-studio-20260722-050000.sqlite
...
恢复必须停止全部 Content Studio 进程，先 dry-run 校验备份完整性与清单哈希：

```bash
npm run database -- restore \
  --database /var/lib/content-studio/content-studio.sqlite \
  --backup /var/backups/content-studio/content-studio-20260722-050000.sqlite
```
...
1. 停止全部 Content Studio 进程。
2. 保留失败数据库作为调查证据，将当前数据库及 sidecar 移出运行路径。
3. 把恢复命令输出的 `.rollback-*` 主文件改回 `CONTENT_STUDIO_DATABASE_FILE`，对应 `-wal`、`-shm` sidecar 同步恢复。
