...
当前版本用于产品演示、交互验证和后续前后端联调，不是 PRD 第 29 节定义的 Production 完成态。画布数据保存在浏览器内存中，Fal 编排任务保存在 Vite 服务进程内存中，刷新页面或重启服务不会提供生产级任务恢复。定向光当前通过通用编辑模型和严格保真提示实现，属于实验能力，建议对品牌字样和材质进行人工复核。尚未接入真实登录/MFA/RBAC、数据库、对象存储、持久化任务队列、签名导出、通知和生产审计基础设施。
