...
describe('企业管理浏览器客户端', () => {
  ...
  it('携带 CSRF 保护创建项目并严格解析返回数据', async () => {
    document.cookie = 'pias_csrf=csrf-token; Path=/';
    ...
    expect((fetcher.mock.calls[0][1].headers as Headers).get('x-pias-csrf')).toBe('csrf-token');
  });
  ...
  it('保留邀请的未投递状态，不把保存写成发送成功', async () => {
    ...
    await expect(createInvitation({
      email: 'reviewer@pias.test', displayName: '', role: 'reviewer', projectIds: ['project-autumn'],
  ...
  it('公开预览和接受邀请，但撤销请求携带 CSRF 保护', async () => {
    ...
    const fetcher = vi.fn()
      .mockResolvedValueOnce(jsonResponse({
        invitation: {
          email: 'member@pias.test', role: 'viewer', projectIds: ['project-autumn'],
    ...
    await expect(previewInvitation(token)).resolves.toMatchObject({ email: 'member@pias.test' });
    await expect(acceptInvitation({ token, password: 'PIAS-member-2026!' }))
    ...
    expect((fetcher.mock.calls[0][1].headers as Headers).get('x-pias-csrf')).toBeNull();
    expect((fetcher.mock.calls[2][1].headers as Headers).get('x-pias-csrf')).toBe('csrf-token');
  });
  ...
  it('重新签发邀请并返回新的单次链接', async () => {
    ...
    const fetcher = vi.fn().mockResolvedValue(jsonResponse({
      invitation: {
        ...
        email: 'resent@pias.test', role: 'viewer', projectIds: ['project-autumn'],
  ...
  it('strictly parses member lists and protects member updates with CSRF', async () => {
    ...
    const member = {
      ...
      email: 'managed@pias.test', displayName: '受管成员', role: 'creator', status: 'active',
      projectIds: ['project-autumn'], mfaEnabled: false,
      createdAt: '2026-07-22T06:30:00.000Z', updatedAt: '2026-07-22T06:30:00.000Z',
    };
    const fetcher = vi.fn()
    ...
    expect((fetcher.mock.calls[1][1].headers as Headers).get('x-pias-csrf')).toBe('csrf-token');
  });
